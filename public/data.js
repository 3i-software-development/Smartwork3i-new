// import {BsFolderSymlink,BsFillFileEarmarkLock2Fill} from 'react-icons/bs';
// import {GiAlarmClock} from 'react-icons/gi';
// import {BiSupport} from 'react-icons/bi';
// import {RiMoneyDollarCircleLine} from 'react-icons/ri';
// import {MdOutlineBuild} from 'react-icons/md';

export const ListMenu = [
    {
        title:'TRANG CHỦ',
        to:'',
        children:[]
    }
    ,
  
    {
        title:'SẢN PHẨM',
        to:'products',
        children:[
           {
            img:`/assets/images/3i/icon-header/logoSW2.png`,
            id:`smartwork`,
            title:`HỆ ĐIỀU HÀNH DOANH NGHIỆP SMARTWORK`   
           },
           {
            img:`/assets/images/3i/icon-header/he-thong-quan-ly.jpg`,
            id:`vicem`,
            title:`Hệ thống quản lý thị trường Vicem`   
           },
           {
            img:`/assets/images/3i/icon-header/4222002.png`,
            id:`acb`,
            title:`Hệ thống quản lý kho, chứng từ và số hóa ACB`   
           },
           {
            img:`/assets/images/3i/icon-header/he-thong-quan-ly.jpg`,
            id:`2707`,
            title:`Hệ thống quản lý sản xuất`   
           },
           {
            img:`/assets/images/3i/icon-header/dispatch-management.png`,
            id:`2708`,
            title:`Hệ thống quản lý công văn và văn bản EDMS`   
           },
        //    {
        //     img:`/assets/images/3i/icon-header/4222002.png`,
        //     id:`2709`,
        //     title:`Hệ thống quản lý kho, chứng từ và số hóa`   
        //    },
           {
            img:`/assets/images/3i/icon-header/6600563.png`,
            id:`2710`,
            title:`Hệ thống quản lý giáo dục thông minh Smart education`   
           },
           {
            img:`/assets/images/3i/icon-header/67-676376_banner-img2-hospital-management-system-icon.png`,
            id:`2711`,
            title:`Hệ thống quản lý phòng khám SMART SURGERY`   
           }
        ]
    },
    {
        title:'DỊCH VỤ',
        to:'services',
        children:[
            {
             img:`/assets/images/3i/icon-header/2154319.png`,
             id:`2712`,
             title:`PHÁT TRIỂN PHẦN MỀM QUỐC TẾ`   
            },
            {
             img:`/assets/images/3i/icon-header/phat-trien-pm-tuybien.png`,
             id:`2714`,
             title:`PHÁT TRIỂN PHẦN MỀM TÙY BIẾN`   
            },
            {
             img:`/assets/images/3i/icon-header/3347960.png`,
             id:`2715`,
             title:`DỊCH VỤ PHẦN MỀM OUTSOURCE`   
            },
            {
             img:`/assets/images/3i/icon-header/1311450.png`,
             id:`2716`,
             title:`PHÁT TRIỂN SẢN PHẨM PHẦN MỀM`   
            },{
             img:`/assets/images/3i/icon-header/1197511.png`,
             id:`2717`,
             title:`PHÁT TRIỂN ỨNG DỤNG WEB`   
            },
            {
             img:`/assets/images/3i/icon-header/dichuyen-csdl.jpg`,
             id:`2718`,
             title:`DỊCH VỤ DI CHUYỂN CSDL`   
            },
            {
             img:`/assets/images/3i/icon-header/3289961.png`,
             id:`2719`,
             title:`PHÁT TRIỂN ỨNG DỤNG DI ĐỘNG`   
            },
            {
             img:`/assets/images/3i/icon-header/7187399.png`,
             id:`2720`,
             title:`KIỂM THỬ PHẦN MỀM QUỐC TẾ`   
            }
         ]
    },
    {
        title:'CÔNG NGHỆ',
        to:'technology',
        children:[
            {
             img:`/assets/images/3i/icon-header/asp.jfif`,
             id:`2727`,
             title:`ASP.NET`   
            },
            {
             img:`/assets/images/3i/icon-header/raw.png`,
             id:`2738`,
             title:`PHP`   
            },
            {
             img:`/assets/images/3i/icon-header/430-4309640_js-logo-nodejs-logo-clipart.png`,
             id:`2740`,
             title:`NodeJS`   
            },
            {
             img:`/assets/images/3i/icon-header/communityIcon_qomtlzo996u61.jpg`,
             id:`2736`,
             title:`Ionic`   
            },{
             img:`/assets/images/3i/icon-header/sql-1024x572.jpg`,
             id:`2737`,
             title:`Ứng dụng vè CSDL và Bigdata`   
            },
            {
             img:`/assets/images/3i/icon-header/2091665.png`,
             id:`2752`,
             title:`Phát triển của Blockchain`   
            },
            {
             img:`/assets/images/3i/icon-header/gg.png`,
             id:`2763`,
             title:`Kiến thức quy trình Test Phần mềm`   
            },
            {
             img:`/assets/images/3i/icon-header/226777.png`,
             id:`2743`,
             title:`JAVA`   
            },
            {
                img:`/assets/images/3i/icon-header/6091352.png`,
                id:`2746`,
                title:`IOT`   
               },
         ]
    },
    {
        title:'TUYỂN DỤNG',
        to:'careers',
        children:[
            {
             img:`/assets/images/3i/icon-header/4412932.png`,
             id:`2724`,
             title:`Tuyển dụng lập trình .NET CORE`   
            },
            {
             img:`/assets/images/3i/icon-header/4419547.png`,
             id:`2725`,
             title:`Tuyển dụng lập trình NODEJS`   
            },
            {
             img:`/assets/images/3i/icon-header/2574733.png`,
             id:`2726`,
             title:`Tuyển dụng lập trình Python`   
            },
            {
             img:`/assets/images/3i/icon-header/1918557.png`,
             id:`2734`,
             title:`Tuyển dụng Tester`   
            }
         ]
    },
    // {
    //     title:'BỘ CÀI',
    //     to:'/dowload',
    //     children:[            
    //      ]
    // },
    // {
    //     title:'TÀI LIỆU',
    //     to:'/documents',
    //     children:[          
    //      ]
    // },
    {
        title:'GIỚI THIỆU',
        to:'about-us',
        children:[
            {
             img:`/assets/images/3i/icon-header/9194815.png`,
             id:`2730`,
             title:`GIỚI THIỆU`   
            }
         ]
    },
    {
        title:'LIÊN HỆ',
        to:'contact',
        children:[
            {
             img:`/assets/images/3i/icon-header/3249834.png`,
             id:`2733`,
             title:`Liên hệ`   
            }
         ]
    }
]

export const ListSlideHome  = [
    // "/assets/Slide/slide.jpg",
    // "/assets/Slide/slide (1).jpg",
    // "/assets/Slide/slide (2).jpg",
    // "/assets/Slide/11 (2).png",
    // "/assets/Slide/6(2).png",
    // "/assets/Slide/1.png",
    // "/assets/Slide/3.png",
    "/assets/images/3i/2.png",
    "/assets/images/3i/3.png",
    "/assets/images/3i/4.png",
    "/assets/images/3i/5.png",
]

export const SmartworkIntro = [
    {
        img:"/assets/listicon/share.png",
        title:'Tích hợp toàn diện',
        content:'SmartWork cung cấp đày đủ các chức năng quan trọng  trên cùng một nền tảng, giúp cho việc quản lý doanh nghiệp trở nên đơn giản và thuận tiện hơn.'
    },
    {
        img:"/assets/listicon/wall-clock.png",
        title:'Nâng cao năng suất',
        content:'Với SmartWork bạn có thể tạo ra các quy trình làm việc, giao việc và theo dõi tiến độ công việc , báo cáo thống kê về năng suất làm việc của từng thành viên trong nhóm để đánh giá hiệu quả công việc của họ.'
    },
    {
        img:"/assets/listicon/telephone-call.png",
        title:'Hỗ trợ 24/7',
        content:'Đội ngũ hỗ trợ chuyên nghiệp, sẵn sàng hỗ trợ bạn mọi lúc mọi nơi. Nếu bạn gặp bất kỳ vấn đề nào trong quá trình sử dụng phần mềm, bạn có thể liên hệ với đội ngũ hỗ trợ để được giải đáp và hỗ trợ kịp thời.'
    },
    {
        img:"/assets/listicon/dollar.png",
        title:'Tiết kiệm chi phí',
        content:'Tiết kiệm chi phí cho doanh nghiệp khi chỉ cần sử dụng một sản phẩm để quản lý nhiều phần khác nhau. Đồng thời, giá thành của SmartWork cũng rất cạnh tranh so với các phần mềm quản lý khác trên thị trường.'
    }
]


export const ListSmartwork = [
    {
        img:"/assets/listphanhe/1.jpg",
        title:'Phân hệ quản trị nhân sự , chấm công và API',
        content:[
            'Quản lý hồ sơ nhân sự cơ bản',
            'Quản lý quá trình làm việc của nhân sự',
            'Quản lý việc chấm công cho nhân viên',
            'Quản lý KPI của nhân sự',
            'Hệ thống số hóa dữ liệu nhân sự chuyên nghiệp'
        ]
    },
    {
        img:"/assets/listphanhe/02.jpg",
        title:'Phân hệ quản lí ngưởi dùng và phân quyền',
        content:[
            'Quản lý tài khoản người dùng hệ thống',
            'Quản lý chi nhánh, phòng ban',
            'Quản lý vai trò từng cá nhân',
            'Quản lý hoạt động của người dùng',
            'Phân quyền cho từng cá nhân'
        ]
    },
    {
        img:"/assets/listphanhe/3.png",
        title:'Phân hệ quản lí kho và bán hàng',
        content:[
            'Chuẩn hóa quy trình bán hàng',
            'Chuẩn hóa quy trình mua hàng,vật tư',
            'Chuẩn hóa quy trình kho vận',
            'Chuẩn hóa quy trình quản lý sản phẩm'
        ]
    },
    {
        img:"/assets/listphanhe/4.jpg",
        title:'Chăm sóc khách hàng và nhà cung cấp CRM',
        content:[
            'Quản lí báo giá ',
            'Quản lí công nợ',
            'Tổng hợp báo cáo thống kê',          
            'Tính năng sau bán hàng',
            'Quản lí đơn hàng',
            'Tìm kiếm khách hàng tiềm năng'
        ]

    },
    {
        img:"/assets/listphanhe/5.jpg",
        title:'Phân hệ chức năng quản trị tài sản',
        content:[
            'Sổ tài sản',
            'Mua sắm , vận hành',
            'Bảo dưỡng , sửa chữa',
            'Danh mục chung'
        ]

    },
    {
        img:"/assets/listphanhe/6.jpg",
        title:'Phân hệ chức năng quản trị tài chính',
        content:[
            'Quản lí quỹ',
            'Cảnh báo quỹ'
        ]

    },
    {
        img:"/assets/listphanhe/7.jpg",
        title:'Phân hệ số hóa tài liệu ,quản trị công văn',
        content:[
            'Chia sẻ thông tin nhanh chóng',
            'Nâng cao hiệu quả công việc',
            'Chi phí vận hành , quản lí thấp',
            'Tăng cường bảo mật',
            'Giảm không gian lưu trữ',
            'Tránh việc mất, nhàu nát tài liệu',
            'Lưu trữ, quản lý tài liệu vĩnh viễn',
            'Giảm thời gian tìm kiếm'
        ]

    },
    {
        img:"/assets/listphanhe/9.jpg",
        title:'Phân hệ quản trị hệ thống , điều hành công việc',
        content:[
            'Đăng kí lịch làm việc',
            'Quản lí lịch làm việc',
            'Lịch phỏng vấn',
            'Thẻ việc'
        ]
    },
    {
        img:"/assets/listphanhe/10.webp",
        title:'Phân hệ CAMERA AI',
        content:[
            'Nhận diện biển số',
            'Nhận diện khuôn mặt',
            'FACE ID kết hợp với cơ sở dữ liệu ERP sẵn có'
        ]

    },
    {
        img:"/assets/listphanhe/11.jpg",
        title:'Phân hệ SUPPORT CENTER',
        content:[
            'Camera thu thập thông tin khách hàng',
            'Tổng hợp lượng quan tâm với từng sản phẩm',
            'Phần mềm phân tích độ hài long với sản phẩm',
            'Supporter có thể trao đổi trực tiếp với khách hàng',
            'Điều hướng màn hình của khách hàng, đưa ra sản phẩm khách hàng cần',
            'Giải quyết mọi nhu cầu, thắc mắc của khách hàng'
        ]

    },
    {
        img:"/assets/listphanhe/12.jpg",
        title:'Phân hệ báo cáo tổng hợp',
        content:[
            'Dữ liệu từ nhiều nguồn khác nhau và tự động kết chuyển thành thông tin tài chính xuất ra báo cáo',
            'Khả năng tùy chỉnh dễ dàng đáp ứng được nhiều yêu cầu đặc thù trong báo cáo tài chính',
            'Cho phép truy xuất dữ liệu đa chiều, nhanh chóng và chính xác',
            'Báo cáo logic, đầy đủ và chi tiết giúp việc theo dõi, đánh giá tình hình tài chính dễ dàng'
        ]

    }
]

export const ListDowload = [
    {
        title:'Hệ thống quản lí doanh nghiệp Smartwork',
        list:[{
            icon:'/assets/listicon/ph2.png',
            title:'Điều hành việc',
            file:['/assets/dowload/Dieuhanhviec_khosohoa.pptx']
        },
        {
            icon:'/assets/listicon/ph11.png',
            title:'Quản lý nhân sự',
            file:['/assets/dowload/Nhansu.pptx']
        },
        {
            icon:'/assets/listicon/ph3.png',
            title:'Quản lý tài sản',
            file:['/assets/dowload/QLTS.pptx']
        },
        {
            icon:'/assets/listicon/ph4.png',
            title:'Kho và bán hàng',
            file:[
                '/assets/dowload/Kho_banhang1.pptx',
                '/assets/dowload/Kho_banhang2.pptx'
            ]
        },
        {
            icon:'/assets/listicon/ph5.png',
            title:'Tài sản và vận hành',
            file:['/assets/dowload/Taisan.pptx']
        },
        {
            icon:'/assets/listicon/ph6.png',
            title:'Hợp đồng và đơn hàng',
            file:['/assets/dowload/Hopdongdonhang.pptx']
        },
        {
            icon:'/assets/listicon/ph7.png',
            title:'Quản trị nhà cung cấp',
            file:['/assets/dowload/Nhacungcap.pptx']
        },
        {
            icon:'/assets/listicon/ph8.png',
            title:'Quản trị khách hàng',
            file:['/assets/dowload/Khachhang.pptx']
        },
        {
            icon:'/assets/listicon/ph9.png',
            title:'Quản trị tài chính',
            file:['/assets/dowload/Taichinh.pptx']
        },
        {
            icon:'/assets/listicon/ph1.png',
            title:'Quản trị hệ thống',
            file:['/assets/dowload/Quantrihethong.pptx']
        },
        {
            icon:'/assets/listicon/ph10.png',
            title:'Đào tạo nội bộ',
            file:['/assets/dowload/Daotaonoibo.pptx']
        }]
    },
    {
        title:'Profile công ty',
        list:[
            {
                icon:'/assets/listicon/ph6.png',
                title:'Profile',
                file:['/assets/dowload/Profile3i.pdf']
            }
        ]       
    },
    {
        title:'Các sản phẩm khác',
        list:[
            {
                icon:'/assets/listicon/vedan.png',
                title:'Hệ thống quản lí xe cộ , vật tư , cư xá Vedan',
                file:['/assets/dowload/Vedan.pptx']
            },
            {
                icon:'/assets/listicon/travel.png',
                title:'Smarttravel',
                file:['/assets/dowload/Smarttravel.pptx']
            },
            {
                icon:'/assets/listicon/crane.png',
                title:'Romooc Tracking',
                file:['/assets/dowload/Romooc.pptx']
            },
            {
                icon:'/assets/listicon/hospital.png',
                title:'Quản lí phòng khám',
                file:['/assets/dowload/QLPK.pdf']
            },
            {
                icon:'/assets/listicon/bank.png',
                title:'RECONCILIATION, PACKAGE PRODUCT VIB',
                file:['/assets/dowload/admin.pptx',
                '/assets/dowload/banca.pptx',
                '/assets/dowload/package.pptx',
            ]
            }
        ]
    }
    
]
